...
class observed_plate_orbit_builder {
 private:
  float radius_prior_;
  float yaw_observation_variance_;
 ...
 public:
  PF_TARGET_ATTRS observed_plate_orbit from_one_plate(const observed_plate& plate_one) const noexcept {
    ...
    const float effective_yaw_variance = thrust::max(1.0e-6f, plate_one.yaw_variance() + yaw_observation_variance_);
    const float yaw_weight = 1.0f / (1.0f + effective_yaw_variance);

    const Eigen::Vector3f predicted_center =
        yaw_weight * yaw_based_center + (1.0f - yaw_weight) * observer_based_center;
  ...
  PF_TARGET_ATTRS
  observed_plate_orbit_builder(
      const float& radius_prior,
      const float& yaw_observation_variance,
      const Eigen::Vector3f& observer_position) noexcept
      : radius_prior_{radius_prior},
        yaw_observation_variance_{yaw_observation_variance},
