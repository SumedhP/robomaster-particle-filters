...
struct most_likely_particle_reduction_impl {
  ...
  struct orientation_and_z_coordinates {
    float orientation;
  ...
  PF_TARGET_ONLY_ATTRS [[nodiscard]] inline state_type operator()(const state_type& a, const state_type& b) const noexcept {
    ...
    const pf::util::device_array<orientation_and_z_coordinates, 5> candidates = {
        orientation_and_z_coordinates{
            a_particle.orientation() + 0.0f * half_pi, a_particle.z_coordinate_0(), a_particle.z_coordinate_1()},
        orientation_and_z_coordinates{
            a_particle.orientation() + 1.0f * half_pi, a_particle.z_coordinate_1(), a_particle.z_coordinate_0()},
        orientation_and_z_coordinates{
            a_particle.orientation() - 1.0f * half_pi, a_particle.z_coordinate_1(), a_particle.z_coordinate_0()},
        orientation_and_z_coordinates{
            a_particle.orientation() + 2.0f * half_pi, a_particle.z_coordinate_0(), a_particle.z_coordinate_1()},
        orientation_and_z_coordinates{
            a_particle.orientation() - 2.0f * half_pi, a_particle.z_coordinate_0(), a_particle.z_coordinate_1()},
    };

    const auto target = b_particle.orientation();
    const auto [a_orientation, a_z_coordinate_0, a_z_coordinate_1] =
    ...
    const auto state =
        prediction(radius, z_coordinate_0, z_coordinate_1, orientation, orientation_velocity, center, center_velocity);
...
class particle_filter_configuration {
 ...
 public:
  ...
  PF_TARGET_ONLY_ATTRS [[nodiscard]] float conditional_log_likelihood(
    ...
    const Eigen::Vector3f observer_delta = state.observer_position() - helper::rpad_zero(given.center());
    ...
    const auto [logit_visibility_0, logit_visibility_1, logit_visibility_2, logit_visibility_3] =
        predicted_plates.transformed([&, this](const predicted_plate& plate) {
          const Eigen::Vector3f plate_delta = plate.position() - helper::rpad_zero(given.center());
  ...
  PF_TARGET_ONLY_ATTRS [[nodiscard]] prediction sample_from(util::default_rv_sampler& sampler, const observation& state)
    ...
    return prediction(radius, z_coordinate_0, z_coordinate_1, orientation, orientation_velocity, center_xy, center_velocity);
  }
