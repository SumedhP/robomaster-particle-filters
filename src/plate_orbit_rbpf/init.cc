...
void init(py::module_& m) noexcept {
  auto plate_orbit_rbpf = m.def_submodule("plate_orbit_rbpf");

  py::class_<observed_plate>(plate_orbit_rbpf, "ObservedPlate")
  ...
  py::class_<prediction>(plate_orbit_rbpf, "Prediction")
      ...
      // New in the RBPF filter: the full 8x8 posterior covariance over the linear substate,
      // ordered as in plate_orbit_rbpf::linear_state.
      .def("covariance", &prediction::covariance_for_host)
      .def("extrapolate_state", &prediction::extrapolate_state);

  py::class_<particle_filter_configuration_parameters>(plate_orbit_rbpf, "ParticleFilterConfigurationParameters")
      .def(py::init<
           float,   // radius_prior
  ...
  py::class_<particle_filter>(plate_orbit_rbpf, "ParticleFilter")
